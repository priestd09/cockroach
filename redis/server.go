...
// An Server provides both an HTTP and RPC server endpoint serving the Redis API.
...
func MakeServer(ctx *base.Context, db client.DB) Server {
	return Server{context: ctx, Executor: newExecutor(db)}
}

// ServeHTTP serves the Redis API by treating the request URL path
...
func (s Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var c driver.Command
	if err := util.UnmarshalRequest(r, reqBody, &c, allowedEncodings); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	// Check request user against client certificate user.
	if err := authenticationHook(&c, true /* public */); err != nil {
		http.Error(w, err.Error(), http.StatusUnauthorized)
		return
	}

	reply, code, err := s.Execute(c)
	if err != nil {
...
// RegisterRPC registers the Redis RPC endpoint.
func (s Server) RegisterRPC(rpcServer *rpc.Server) error {
	return rpcServer.RegisterPublic(driver.RPCMethod, s.executeCmd, &driver.Command{})
}

func (s Server) executeCmd(argsI proto.Message) (proto.Message, error) {
	args := argsI.(*driver.Command)
