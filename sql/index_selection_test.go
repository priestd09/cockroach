...
func TestMakeConstraints(t *testing.T) {
	...
	testData := []struct {
		...
		*/
		...
		{`ST_DISTANCE(l, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < .1`, `l`, `[l >= 5/20: '{"type":"Point","coordinates":[-73.97536754608154,40.751255880455915]}', l <= 5/21: '{"type":"Point","coordinates":[-73.97536754608154,40.751255880455915]}']`},
	}
...
func TestMakeSpans(t *testing.T) {
	...
	testData := []struct {
		...
		{`ST_DISTANCE(l, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < .0000001`, `l`, `/-5693121811630260224-/-5693121811613483008`, `/-5693121811621871616-/-5693121811638648832`},
		// Verify a query without an index on l does a full scan.
		//{`ST_DISTANCE(l, ST_GeogFromGeoJSON('{"type":"Point","coordinates":[-73.97536754608154, 40.751255880455915]}')) < 2`, `a`, `-`, `-`},
	}
	for _, d := range testData {
		for _, dir := range []encoding.Direction{encoding.Ascending, encoding.Descending} {
			...
			if expected != s {
				t.Errorf("[index direction: %d] %s: expected %s, but found %q", dir, d.expr, expected, s)
			}
		}
	}
