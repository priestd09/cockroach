...
var (
	flagCockroachAddr = flag.String("cockroach", "localhost:26257", "CockroachDB address")
	flagListenAddr    = flag.String("addr", "localhost:26379", "Redis listen address")
)

func main() {
	conn, err := grpc.Dial(*flagCockroachAddr, grpc.WithInsecure())
	if err != nil {
		log.Fatal(err)
	}
	log.Printf("connected to CockroachDB at %s", *flagCockroachAddr)
