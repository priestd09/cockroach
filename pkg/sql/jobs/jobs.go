...
type ProgressedFn func(ctx context.Context, details interface{}) (float32, error)

// SimpleProgress returns a ProgressedFn that returns progress.
func SimpleProgress(progress float32) ProgressedFn {
	return func(ctx context.Context, details interface{}) (float32, error) {
		return progress, nil
	}
}

// Progressed updates the progress of the tracked job
// to fractionCompleted. progressedFn is a transaction-safe function
// that returns the current progress, possibly based on the job details,
// which it can also modify and will be saved. If it return less than the
// currently-recorded fractionCompleted, it will be silently ignored.
func (j *Job) Progressed(ctx context.Context, progressedFn ProgressedFn) error {
	return j.update(ctx, func(status *Status, payload *Payload) (bool, error) {
		...
		fractionCompleted, err := progressedFn(ctx, payload.Details)
		if err != nil {
			return false, err
		}
